...
tags: ['Data Science', 'Experiment', 'A B Testing', 'Meta Analysis', 'Statistical Inference']
---
...
- Improperly calling experiments early (such as by [HARKing](https://www.ncbi.nlm.nih.gov/pubmed/15647155) or [p-hacking](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3204791)) may substantially inflate false positive rates and consequently lead to wasted business effort.
...
The secret sauce we used here is [meta-analysis](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3049418/), a simple yet powerful method of analyzing related analyses. We adopted this methodology to identify time-varying treatment effects. One frequent application of this method in healthcare is to combine results from independent studies to boost power and improve estimates of treatment effects, such as the efficacy of a new drug. At a high level:
...
![image](../images/bf9b1cf541681abe.png)

More details can be found in [this reference](https://www.springer.com/us/book/9783319214153). The model-fitting process (i.e. fixed-effect model versus random-effect model) can be leveraged to test whether heterogeneous treatment effects are present across time dimensions (e.g., _time of day, day of week, week over week, pre-/post-event_). We conducted a comprehensive retrospective study in A/B tests on the signup flow. As expected, we found most tests do not demonstrate strong heterogeneous treatment effects over time. Therefore, we could have ended some tests early, innovated more, and brought an even better experience to our prospective customers sooner.

### End Experiments Early with Optimal Stopping

Assuming that a treatment effect is both time-invariant (evaluated by meta-analysis) and sufficiently large, we can apply various optimal-stopping strategies to end tests early. Naively, we could constantly peek at experiment dashboards, but this will inflate false positives when we mistakenly believe a treatment effect is present. There are scientific methodologies to control for false positives (Type I errors) with peeking (or, more formally, interim analyses). Several methods have been assessed in our retrospective study, such as [Wald’s Sequential Probability Ratio Tests (SPRT)](https://en.wikipedia.org/wiki/Sequential_probability_ratio_test), [Sequential Triangular Testing](https://www.tandfonline.com/doi/abs/10.1080/15598608.2016.1268549?journalCode=ujsp20), and [Group Sequential Testing (GST)](https://en.wikipedia.org/wiki/Sequential_analysis). GST demonstrated the best performance and practical value in our study; it is widely used in clinical trials in which samples are accumulated over time in batches, which is a perfect fit our use case. This is roughly how it works:

- Before a test starts, we decide the minimum required running time and the number of interim analyses.
- GST then _allocates_ the total tolerable Type I error (for example, 0.05) into all interim analyses, such that the total Type I error sums up to the overall Type I error. As a result, each interim test becomes more conservative than regular peeking.
...
![image](../images/421845c9de7b51cb.png)
...
![image](../images/9350c7da6f7381a3.png)

### Building Decision Support into Our Experimentation Platform

Now that our initial research is completed, we are actively developing meta analysis, optimal stopping, heterogeneity treatment effects detection, and much more into the larger Netflix [Experimentation and Causal Inference platform](https://research.netflix.com/research-area/machine-learning-platform). We hope such features will accelerate our current experimentation workflow, expedite product innovation, and ultimately bring the best experience and delight to our customers. This is an ongoing journey, and if you are passionate about our mission and our exciting work, [join our all-star team](https://research.netflix.com/jobs?page=1&limit=20&area=Experimentation+and+Causal+Inference%7EML+and+Experimentation+Platform)!
