...
tags: ['Microservices', 'Netflix', 'Video Encoding', 'Streaming']
---
...
_This is the first blog in a multi-part series on how Netflix rebuilt its video processing pipeline with microservices, so we can maintain our rapid pace of innovation and continuously improve the system for member streaming and studio operations. This introductory blog focuses on an overview of our journey. Future blogs will provide deeper dives into each service, sharing insights and lessons learned from this process._
...
- We moved from centralized linear encoding to [distributed chunk-based encoding](https://netflixtechblog.com/high-quality-video-encoding-at-scale-d159db052746). This architecture shift greatly reduced the processing latency and increased system resiliency.
- Moving away from the use of dedicated instances that were constrained in quantity, we tapped into Netflix’s [internal trough](https://netflixtechblog.com/creating-your-own-ec2-spot-market-6dd001875f5) created due to autoscaling microservices, leading to significant improvements in computation elasticity as well as resource utilization efficiency.
- We rolled out encoding innovations such as [per-title](https://medium.com/netflix-techblog/per-title-encode-optimization-7e99442b62a2) and [per-shot](https://netflixtechblog.com/optimized-shot-based-encodes-now-streaming-4b9464204830) optimizations, which provided significant quality-of-experience (QoE) improvement to Netflix members.
...
3. calculate the quality score ([VMAF](https://netflixtechblog.com/vmaf-the-journey-continues-44b51ee9ed12)) of each chunk
...
Thus, in Cosmos, we created two independent microservices: Video Encoding Service (VES) and [Video Quality Service (VQS)](./netflix-video-quality-at-scale-with-cosmos-microservices-552be631c113.md), each of which serves a clear, decoupled function. **As implementation details, the chunked encoding and the assembling were abstracted away into the VES**.
...
6. [Video Quality Service (VQS)](./netflix-video-quality-at-scale-with-cosmos-microservices-552be631c113.md): This service takes the mezzanine and the encoded video as input, and calculates the quality score (VMAF) of the encoded video.
...
![image](../images/4dbf93c9a133a8f0.png)
